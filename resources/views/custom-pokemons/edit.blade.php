...
<head>
    ...
    <title>Editar {{ $pokemon->name }} — Custom Pokédex</title>
    ...
    <style>
        ...
        .current-sprite-box { background: #0d0d1a; border: 1px dashed #1e2e4a; border-radius: 6px; padding: 10px; margin-bottom: 10px; text-align: center; }
    </style>
</head>
...
<main class="max-w-6xl mx-auto px-4 py-8">
    ...
    <div class="flex items-center gap-4 mb-6 flex-wrap">
        <a href="{{ route('custom-pokemons.show', $pokemon->id) }}" class="action-btn px-4 py-2 text-xs rounded">← VOLTAR</a>
        <h1 class="text-blue-400" style="font-size: 14px;">EDITAR POKÉMON</h1>
    </div>

    @if ($errors->any())
        <div class="mb-5 px-4 py-3 rounded" style="background: rgba(255,68,68,0.1); border: 2px solid #ff4444; color: #ff6666; font-size: 7px;">
    ...
    <form method="POST" action="{{ route('custom-pokemons.update', $pokemon->id) }}" enctype="multipart/form-data" novalidate id="edit-form">
        @csrf
        @method('PUT')
                    ...
                    <div class="panel-body text-center">
                        ...
                        <div class="mb-4" style="background: #9bbc0f; border-radius: 8px; padding: 16px; position: relative;">
                            ...
                            <img id="preview-sprite"
                                 src="{{ $pokemon->sprite_path ? asset($pokemon->sprite_path) : 'https://placehold.co/96x96/9bbc0f/1a1a2e?text=' . urlencode(strtoupper($pokemon->name[0])) }}"
                                 alt="preview" class="w-24 h-24 mx-auto object-contain relative"
                                 style="image-rendering: pixelated;"
                                 onerror="this.src='https://placehold.co/96x96/9bbc0f/1a1a2e?text=?'">
                        </div>

                        {{-- Name preview --}}
                        <p id="preview-name" class="text-green-400 mb-2" style="font-size: 10px; min-height: 18px;">{{ strtoupper(old('name', $pokemon->name)) }}</p>
                        <p id="preview-dex" class="text-gray-600 mb-3" style="font-size: 6px;">#{{ str_pad(old('dex_number', $pokemon->dex_number), 4, '0', STR_PAD_LEFT) }}</p>
                            ...
                            @foreach(['hp' => 'HP', 'attack' => 'ATK', 'defense' => 'DEF', 'speed' => 'VEL'] as $key => $label)
                                <div class="flex items-center gap-2">
                                    ...
                                    <div class="flex-1 stat-preview">
                                        <div class="stat-preview-fill" id="preview-stat-{{ $key }}" style="width: {{ (old($key, $pokemon->$key)/255)*100 }}%;"></div>
                                    </div>
                                    <span class="text-gray-500 flex-shrink-0" id="preview-val-{{ $key }}" style="font-size: 5px; width: 20px; text-align: right;">{{ old($key, $pokemon->$key) }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="lg:col-span-2 space-y-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="number" id="dex_number" name="dex_number"
                                       ...
                                       value="{{ old('dex_number', $pokemon->dex_number) }}"
                                       oninput="updatePreview()">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select id="type_primary" name="type_primary"
                                    ...
                                    @foreach(['fire','water','grass','electric','ice','fighting','poison','ground','flying','psychic','bug','rock','ghost','dragon','dark','steel','fairy','normal'] as $type)
                                        <option value="{{ $type }}" {{ old('type_primary', $pokemon->type_primary) === $type ? 'selected' : '' }}>
                                            {{ strtoupper($type) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="type_secondary" name="type_secondary"
                                    ...
                                    @foreach(['fire','water','grass','electric','ice','fighting','poison','ground','flying','psychic','bug','rock','ghost','dragon','dark','steel','fairy','normal'] as $type)
                                        <option value="{{ $type }}" {{ old('type_secondary', $pokemon->type_secondary) === $type ? 'selected' : '' }}>
                                            {{ strtoupper($type) }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" id="base_animal" name="base_animal"
                                       ...
                                       value="{{ old('base_animal', $pokemon->base_animal) }}">
                            ...
                            <div>
                                ...
                                <input type="text" id="inspiration" name="inspiration"
                                       ...
                                       value="{{ old('inspiration', $pokemon->inspiration) }}">
                            ...
                            @foreach(['hp' => 'HP', 'attack' => 'ATAQUE', 'defense' => 'DEFESA', 'speed' => 'VELOCIDADE'] as $field => $label)
                                <div>
                                    ...
                                    <input type="number" id="{{ $field }}" name="{{ $field }}"
                                           ...
                                           value="{{ old($field, $pokemon->$field) }}" min="1" max="255"
                                           oninput="updatePreview()">
                ...
                <div class="info-panel">
                    ...
                    <div class="panel-body space-y-4">
                        @php
                            $atkColors    = ['#FF5959','#F5AC78','#9DB7F5','#A7DB8D'];
                            $existingAtks = old('attacks', $pokemon->attacks ?? []);
                        @endphp
                        @for($i = 0; $i < 4; $i++)
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input type="text"
                                           ...
                                           class="pixel-input rounded"
                                           value="{{ $existingAtks[$i]['description'] ?? '' }}"
                ...
                {{-- Sprite --}}
                <div class="info-panel">
                    <div class="panel-header">SPRITE</div>
                    <div class="panel-body">
                        @if($pokemon->sprite_path)
                        <div class="current-sprite-box">
                            <p style="font-size: 5px; color: #4a5568; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 1px;">SPRITE ATUAL</p>
                            <img src="{{ asset($pokemon->sprite_path) }}"
                                 alt="sprite atual"
                                 class="w-16 h-16 mx-auto object-contain"
                                 style="image-rendering: pixelated;"
                                 onerror="this.parentElement.style.display='none'">
                            <p style="font-size: 5px; color: #4a5568; margin-top: 6px;">Envie uma nova imagem para substituir</p>
                        </div>
                        @endif
                        <label for="sprite_file">{{ $pokemon->sprite_path ? 'NOVA IMAGEM (OPCIONAL)' : 'IMAGEM DO POKÉMON (OPCIONAL)' }}</label>
                ...
                <div class="flex justify-between items-center">
                    <a href="{{ route('custom-pokemons.show', $pokemon->id) }}" class="action-btn px-6 py-4 text-xs rounded">
                        ← CANCELAR
                    </a>
                    <button type="submit" class="action-btn blue px-8 py-4 text-xs rounded">
                        SALVAR ALTERAÇÕES →
                    </button>
                </div>
            </div>
